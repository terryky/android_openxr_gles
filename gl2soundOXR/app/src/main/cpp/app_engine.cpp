...
AppEngine::PollActions()
{
    ...
    if (m_input.clickA)
    {
        m_oboePlayer->enable (true);
    }
    else
    {
        m_oboePlayer->enable (false);
    }

    {
        float ratio = m_input.triggerVal[Side::RIGHT];
        float freq  = 440.0f + ratio * 440.0f;
        m_oboePlayer->setFrequency (freq); 
    }
}
